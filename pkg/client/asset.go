...
func (g *GrpcClient) GetAssetIssueByAccount(address string) (*api.AssetIssueList, error) {
	...
	account.Address, err = common.DecodeBase58(address)
	if err != nil {
		return nil, err
	}
...
func (g *GrpcClient) AssetIssue(
	...
	for key, value := range frozenSupply {
		...
		assetIssueContractFrozenSupply := new(core.AssetIssueContract_FrozenSupply)
		assetIssueContractFrozenSupply.FrozenAmount = amount
		assetIssueContractFrozenSupply.FrozenDays = days

		// add supply to contract
		contract.FrozenSupply = append(contract.FrozenSupply, assetIssueContractFrozenSupply)
	}
...
func (g *GrpcClient) ParticipateAssetIssue(from, issuerAddress, tokenID string, amount int64) (*api.TransactionExtention, error) {
	...
	if contract.ToAddress, err = common.DecodeBase58(issuerAddress); err != nil {
		return nil, err
	}
